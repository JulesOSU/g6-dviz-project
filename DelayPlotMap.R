...
plot_df <- plot_df %>%
  mutate(mean_delay_f = as_factor(round_any(plot_df$mean_delay,10,floor)))
...
ggplot() + geom_polygon(data = US, aes(x = long, y = lat, group = group), 
  ...
  scale_color_viridis_c(option = "plasma",
                        breaks = c(min(plot_df$mean_delay), max(plot_df$mean_delay)),
                        labels = c("3 minutes early","30 minutes late"),
...
ggsave("FlightMap2.png", height = 4, width = 10, bg = "#121212")
